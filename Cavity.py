...
def set_BCs(u, v, u_lid = 1.0):
    ## Horizontal Faces:
    # Note that v pts are located at horizontal faces, but u pts are not,
    # therefore, u GHOST nodes must be used to enforce the BCs, while
    # v use simple BOUNDARY nodes

    # Top wall
    v[-1, :] = 0.0  # BOUNDARY nodes

    # We want u[y = 2] = 1, but...
    # u[-1] is at y = 2 + dy/2, a GHOST node
    # u[-2] is at y = 2 - dy/2, an INTERIOR node
    # therefore, if we were to interpolate to y = 2...
    # u_avg = (u[-1] + u[-2])/2 = u_lid, and rearranging...
    # u[-1] = 2.0 - u[-2]. Or, more generally, if the lid velocity is
    # u_lid, then u[-1] = 2*u_lid - u[-2]
    u[-1, :] = 2.0*u_lid - u[-2, :] # GHOST node
    # u_avg[-1, :] = u_lid

    # Bottom wall
    v[0, :] = 0.0 # BOUNDARY node
    u[0, :] = - u[1, :] # GHOST node
    # u_avg[0, :] = 0.0

    ## Vertical Faces:
    # Note that u pts are located at vertical faces, but v pts are not,
    # therefore, v GHOST nodes must be used to enforce the BCs, while
    # u use simple BOUNDARY nodes

    # Left wall
    u[:, 0]  = 0.0 # BOUNDARY node
    v[:, 0]  = - v[:, 1] # GHOST node
    # v_avg[:, 0] = 0.0

    # Right wall
    u[:, -1] = 0.0 # BOUNDARY node
    v[:, -1] = - v[:, -2] # GHOST node
    # v_avg[:, -1] = 0.0
...
# Define parameters.
L=2  # Physical length of the domain.
n = 80 # Number of cell columns/rows.
rho = 1.0 # Density.
nu = 0.1 # Kinematic viscosity.
tf = 20.0 # final simulation time
cfl = 0.1 # CFL number
tol = 1.0e-5  # Poisson solver tolerance threshold value.
maxiter = 1e3 # Max number of iterations on the poisson solver.

# Cell lengths. 
dx = L / (n - 1) # Horizontal cell length.
dy = dx # Vertical cell length.

# ICs.
u = numpy.zeros((n+2, n+1))
v = numpy.zeros((n+1, n+2))
p = np.zeros((n+2, n+2)) + 1e-20
...
fig.suptitle(f'cells={n}, tol={tol}, maxiter={maxiter}, final time: {tf}s')
